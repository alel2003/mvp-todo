import useTaskStore from "@/modules/dashboard/store/task"

import type { TaskMenuEditTitleButtonProps } from "@/modules/dashboard/types/task"

export default function TaskMenuEditTitleButton({
  setIsFormTitleUi
}: TaskMenuEditTitleButtonProps) {
  const { setTaskIdPomodoro } = useTaskStore()

  const handleClick = () => {
    setIsFormTitleUi(true)
    setTaskIdPomodoro(null)
  }

  return (
    <button
      type="button"
      onClick={handleClick}
      className="w-full cursor-pointer rounded px-2 py-1 text-left hover:bg-gray-100">
      Edit Title
    </button>
  )
}
